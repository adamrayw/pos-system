...
function ModalTransaksiBulanIni() {
    ...
    useEffect(() => {
        const fetchTransaksi = async () => {
            ...
            await useApiRequest('transaksi/get/' + JSON.parse(localStorage.getItem('user')).id)
                .then((response) => {
                    setData(response.response.data.transaction_month.Transaksi)
                    setIsLoading(false)
                })
                .catch((error) => {
                    console.log(error)
    ...
    return (
        <>
            ...
            <input type="checkbox" id="modal_month" className="modal-toggle" />
            <div className="modal">
                <div className="modal-box">
                    <h3 className="font-bold text-lg mb-4">Produk Paling Laku Bulan Ini</h3>
                    ...
                    <div className="modal-action">
                        <label htmlFor="modal_month" className="btn bg-blue-500 hover:bg-blue-600 transition border-0">Close</label>
                    </div>
                </div>
            </div>
        </>
...
export default ModalTransaksiBulanIni